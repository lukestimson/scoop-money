...
function BreakdownBar({
  ...
  return (
    <div className="group relative flex flex-1 flex-col items-center">
      <div className="mb-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-zinc-600 dark:text-zinc-300">
        {label}
      </div>
      ...
      <div className="mt-2 text-xs font-semibold tabular-nums text-zinc-700 dark:text-zinc-200">{pct}%</div>

      <div className="pointer-events-none absolute left-1/2 top-5 z-20 w-28 -translate-x-1/2 rounded-md border border-white/20 bg-white/45 px-2 py-1.5 text-[10px] text-zinc-900 opacity-0 shadow-sm backdrop-blur-sm transition-opacity duration-150 group-hover:opacity-100 dark:border-zinc-200/20 dark:bg-zinc-950/40 dark:text-zinc-100">
        <div className="mb-1 text-[9px] font-semibold uppercase tracking-[0.14em] text-zinc-800 dark:text-zinc-200">
          {label}
        </div>
        <div className="flex items-center justify-between gap-2">
          <span className="text-zinc-700/90 dark:text-zinc-300/90">Budget:</span>
...
export function SpendingBreakdownWidget({
  ...
  useEffect(() => {
    ...
    Promise.all([window.api.getTransactions(), window.api.getBudgetItems(budgetType)])
      .then(([nextTx, nextBudget]) => {
        if (cancelled) return
  ...
  const metrics = useMemo(
    () => computeSpendingBreakdown(transactions, budgetItems, budgetType, period, start, end),
    [transactions, budgetItems, budgetType, period, start, end]
  )

  return (
    <div className="flex h-full w-full flex-col" role="region" aria-label="Spending breakdown">
      <h2 className="mb-3 text-sm font-semibold uppercase tracking-[0.12em] text-zinc-700 dark:text-zinc-200">
        Spending Breakdown
      </h2>
      ...
      {loading ? (
        ...
        <div className="flex flex-1 items-end justify-center gap-6 pb-3">
          <BreakdownBar label="Needs" spent={metrics.needsSpent} budget={metrics.needsBudget} tone="needs" />
          <BreakdownBar label="Wants" spent={metrics.wantsSpent} budget={metrics.wantsBudget} tone="wants" />
        </div>
      )}
    </div>
  )
